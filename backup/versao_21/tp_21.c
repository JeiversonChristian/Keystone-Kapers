...
void inicializar_structs(Tecla *teclas, Personagem *policial, ALLEGRO_BITMAP *imagem_policial, Personagem *ladrao, ALLEGRO_BITMAP *imagem_ladrao, Mundo *mundo, ALLEGRO_BITMAP *imagem_cidade, ALLEGRO_BITMAP *imagem_policial_vitorioso, ALLEGRO_BITMAP *imagem_ladrao_vitorioso){
	int i;
	int j;
...
void verificar_interacoes(Personagem *policial, Personagem *ladrao, Tecla teclas, Mundo mundo, int *tempo_captura, int tempo_simulado){
	int i;
	int j;
...
void desenhar_tela_final(Personagem policial, Personagem ladrao, int tempo_captura, ALLEGRO_FONT *font){
	...
	while(playing) 
	{
		...
		if(ev.type == ALLEGRO_EVENT_TIMER) {
			...
			if (policial.ganhou == 0 && ladrao.ganhou == 0) {
				...
				if (pause == 0) {
					// verifica interações
					verificar_interacoes(&policial, &ladrao, teclas, mundo, &tempo_captura, tempo_simulado);
